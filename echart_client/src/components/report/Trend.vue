...
export default {
  ...
  computed: {
    ...
    // 点击过后需要显示的数组
    selectTypes() {
      // console.log('type',this.allData.type);
      if (!this.allData) return []
      // 过度掉当前选中的 类别
      // return this.allData.type.filter(item => item.key !== this.activeName)
	  return this.type.filter(item => item.key !== this.activeName)
    },
    // 显示的标题
    showTitle() {
      if (!this.allData) return ''
      return this.allData.filter(item=>item.type === this.activeName)[0].title
    },
    // 设置给标题的样式
    comStyle() {
      return {
  ...
  methods: {
    ...
    updateChart() {
      ...
      // y轴数据 series下的数据
      const  typeObject=this.allData.filter((item)=>{
        return item.type === this.activeName
      })

      const valueArr = typeObject[0].data
    ...
    screenAdapter() {
      // 测试算出来的 合适的字体大小
     let el = this.$refs.trendRef
      // console.log(el.offsetHeight, el.offsetWidth)
      let offsetWidth = el.offsetHeight / el.offsetWidth > 0.4 ? el.offsetWidth : el.offsetHeight
      this.titleFontSize = offsetWidth / 100 * 3.6
