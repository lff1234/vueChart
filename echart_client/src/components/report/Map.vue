  ...
  methods: {
    ...
    async initChart() {
      ...
      if (!this.chinaMapData) {
        ...
        const { data: res } = await this.axiosInstance.get('/api/chinamap')
      ...
      this.chartInstance.on('click', async e => {
        ...
        if (!this.cityMapData[ProvinceInfo.key]) {
          ...
          const { data: res } = await this.axiosInstance.get('/api/provincemap/'+ProvinceInfo.path)
          // 把请求到的数据保存下来
          this.cityMapData[ProvinceInfo.key] = res.data
          // 注册点击的地图
